	...
	$$
					SELECT $2::bigint, $1, array_agg(f.inter)
						FROM  (SELECT c2.geom ,  varnum_senges.u_overlap_sub(c2.fid, ST_Intersection($1, c2.geom))as inter
								FROM varnum_senges.rpg53_2015  as c2
...
-- Select varnum_senges.u_overlap(geom) from varnum_senges.rpg53_2015 ORDER BY surf_parc desc LIMIT 1000 ;

--____Début traitement_____

--corrections des non-valide
UPDATE 	varnum_senges. rpg53_2015 SET geom = ST_MakeValid(geom) where fid in (
select c1.fid from
	varnum_senges. rpg53_2015 as c1 where
...
-- repérage et enregistrement des intersections
	insert into varnum_senges. atlas (dic) (
		select (varnum_senges.u_overlap(geom, fid)) FROM (select geom, fid from varnum_senges.rpg53_2015 /* LIMIT 1000 */) a1
	-- 	select (varnum_senges.u_overlap(geom, fid)).* FROM (select geom, fid from varnum_senges.rpg53_2015 LIMIT 10) a1     --! Syntaxe de séparation des colones
		);
