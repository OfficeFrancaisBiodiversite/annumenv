...
UPDate varnum_senges. rpg53_2019
	...
	$$
					SELECT $2::bigint, $1, array_agg(f.inter)
						FROM  (SELECT c2.geom ,  varnum_senges.u_overlap_sub(c2.fid, ST_Intersection($1, c2.geom))as inter
								FROM varnum_senges.rpg53_2019  as c2
...
UPDATE 	varnum_senges. rpg53_2019 SET geom = ST_MakeValid(geom) where fid in (
select c1.fid from
	varnum_senges. rpg53_2019 as c1 where
...
DROP INDEX IF EXISTS _19_inter_idx;
DROP INDEX IF EXISTS _19_inter_fid_idx ;

CREATE INDEX  _19_inter_fid_idx ON varnum_senges. ann_inter USING btree(choix) WITH (fillfactor = 100);
CREATE INDEX _19_inter_idx ON varnum_senges. ann_inter USING gist (geom);
...
--
-- ALTER TABLE varnum_senges. test
-- Rename to varnum_senges. RPG_2019_56c;
